...
\hline
\textbf{Method} & \textbf{Original Size} & \textbf{Compressed Size} & \textbf{Compression Ratio} & \textbf{Exec Time}\\
\hline
Polynomial & 2.5 MB & 2.30 MB & 8.0\% & 132 + 211 ms\\
Inter-Channel & 2.5 MB & 2.29 MB & 8.4\% & 111 + 180 ms\\
Lossy & 2.5 MB & 1.01 MB & 59.6\% & 72 + 144 ms\\
\hline
\end{tabular}
...
\textbf{Method} & \textbf{Original Size} & \textbf{Compressed Size} & \textbf{Compression Ratio} & \textbf{Exec Time}\\
\hline
Polynomial & 5.2 MB & 4.27 MB & 17.9\% & 224 + 422 ms\\
Inter-Channel & 5.2 MB & 4.41 MB & 15.2\% & 227 + 362 ms\\
Lossy & 5.2 MB & 1.69 MB & 67.5\% & 137 + 270 ms\\
\hline
\end{tabular}
\caption{Compression Performance Comparison}
\end{table}
...
As we can see, there is no noticeable compression difference between inter-channel coding and predictive coding in the lossless category. However, the lossy codec has a noticeable difference in compression size while also reducing the computation time. The problem rests in the noise generated, but since we only used 8 bitrate (half of the original), it is not noticeable.
